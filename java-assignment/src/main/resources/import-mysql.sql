-- Seed data for the optional MySQL profile. Identical rows to import.sql, but MySQL has no
-- sequences: Hibernate emulates them with one table per sequence, so the reset is an UPDATE
-- instead of an ALTER SEQUENCE.
...
UPDATE Store_SEQ SET next_val = 101;
...
UPDATE Product_SEQ SET next_val = 101;
...
UPDATE warehouse_SEQ SET next_val = 101;
